...
Each generated project includes a CLI tool named `src`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run src server run --reload     # Start dev server
uv run src server routes           # Show all routes
# Database commands
uv run src db migrate -m "message" # Create migration
uv run src db upgrade              # Apply migrations
uv run src db downgrade            # Rollback migration
# User commands
uv run src user create-admin       # Create admin user
uv run src user create             # Create regular user
uv run src user list               # List all users
```
...
# Server
uv run src server run --reload
uv run src server routes

# Database (Alembic wrapper)
uv run src db init
uv run src db migrate -m "Add users"
uv run src db upgrade

# Users
uv run src user create-admin       # Create admin (interactive)
uv run src user create             # Create user (interactive)
uv run src user list               # List all users
```
...
```bash
uv run src cmd seed --count 100
uv run src cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend src server run --reload` |
| Start prod server | `uv run --directory backend src server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend src server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend src db migrate -m "message"` |
| Apply migrations | `uv run --directory backend src db upgrade` |
| Rollback migration | `uv run --directory backend src db downgrade` |
| Show current | `uv run --directory backend src db current` |
| Show history | `uv run --directory backend src db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend src user create-admin` |
| Create user | `uv run --directory backend src user create` |
| List users | `uv run --directory backend src user list` |
